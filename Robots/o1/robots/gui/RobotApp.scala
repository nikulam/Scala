...
private object RobotProgram {
   def makeInitialWorld = {
    ...
    assert(world.width == floorW + 2 && world.height == floorH + 2, s"The created world has width=${world.width} and height=${world.height}. It should have had width=${floorW+2} and height=${floorH+2} (which includes surrounding wall squares).")
    world    
  }
}
