...
abstract class Scenario(val name: String) {
  ...
  protected def requestParameter(prompt: String, min: Int, locator: Component) = {
    requestInt(prompt, _ >= min, "Please enter an integer no lower than " + min + ".", RelativeTo(locator))
  }
...
object Scenario {
  ...
  abstract class MapScenario(name: String) extends Scenario(name) {
    ...
    def setup(locator: Component) = {
      val world = new RobotWorld(this.map(0).size, this.map.size)
      this.addWalls(world)
      this.addBots(world)
      Some(world)
    }
  ...
  object Bar extends MapScenario("Bar") {
    ...
    protected def addBots(world: RobotWorld) = {
      ...
      for (loc <- flyLocs) {
        ...
        fly.brain = Staggerbot("Fly " + flyCount, fly, new Integer(flyCount))
      }
    }
  }
  ...
  object Verona extends MapScenario("Verona") {
    ...
    protected def addBots(world: RobotWorld) = {
      ...
      romeo.brain  = Staggerbot("Romeo", romeo, new Integer(4))
  ...
  object OneOfEach extends MapScenario("One of each") {
    ...
    protected def addBots(world: RobotWorld) = {
      ...
      daniel.brain = Staggerbot("Daniel", daniel, new Integer(133))
    }
  } 
  ...
  abstract class RandomScenario(name: String) extends Scenario(name) {
    ...
    def makeDirection() = {
      CompassDir.Clockwise(util.Random.nextInt(CompassDir.Clockwise.size))
    }

  }

  abstract class BasicRandomScenario(name: String, val acceptedTypes: Seq[RobotType]) extends RandomScenario(name) {
  ...
  class RandomCustom(name: String, acceptedTypes: Seq[RobotType]) extends BasicRandomScenario(name, acceptedTypes) {
