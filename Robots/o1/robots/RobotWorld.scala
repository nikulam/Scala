...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2) {  
  ...
  private def initialSquare(x: Int, y: Int) = if (x > 0 && x < this.width - 1 && y > 0 && y < this.height - 1) new Floor else Wall
  ...
  /** Creates a new robot into this robot world. The newly created robot body does not yet
    ...
    * and informing the robot's initial square that the robot is now there.
    *
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) = {
    val newRobot = new RobotBody(this, initialLocation, initialFacing)
    this.robots += newRobot
    this(initialLocation).addRobot(newRobot)
    newRobot
  }
  ...
  def advanceTurn() = {
    for (next <- this.nextRobot) {
      next.takeTurn()
      this.turn = (this.turn + 1) % this.numberOfRobots
    }
  }
  ...
  /** Causes all the robots in the world to take a turn, starting with the one whose turn it is next.
    ...
    * @see [[RobotBody.takeTurn]] */
  def advanceFullTurn() = {
    for (counter <- 1 to this.numberOfRobots) {
      this.advanceTurn()
    }
  }
