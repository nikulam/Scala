...
object factory {
  ...
  def buildEqual(bus1: Bus, bus2: Bus): Gate = {
    ...
    val vek = bus1.&(bus2).|(((bus1.unary_~)).&((bus2.unary_~)))
    vek.reduceLeft(new AndGate(_, _))
  }
   ...
   *
   */
  private def buildGategi(b1i: Gate, b2i: Gate, giminus1: Gate): Gate = {
 ...
 def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    for(i <- 0 until bus1.length) {
      gate = buildGategi(bus1(i), bus2(i), gate)
    } 
    gate
  }
  ...
  def buildUnsignedLessOrEqual(bus1: Bus, bus2: Bus): Gate = {
    buildEqual(bus1, bus2) || buildUnsignedLess(bus1, bus2)
  }
  ...
  def buildIncrementer(bus: Bus): Bus = {
    require(bus.length > 0, "Cannot build incrementer of an empty bus")  
     val arr = new Array[Gate](bus.length)
     var c_in = Gate.True
     
     for(i <- 0 until bus.length) {
       val (c_out, s) = buildHalfAdder(bus(i), c_in)
       c_in = c_out
       arr(i) = s
     }
     new Bus(arr)
