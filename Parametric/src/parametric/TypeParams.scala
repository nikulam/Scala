...
object TypeParams extends App {
  ...
  def orderPair[T](pair: (T, T))(implicit ordering: Ordering[T]): (T, T) = {
    if(ordering.gt(pair._2, pair._1)) pair
  ...
  def firstSame[T](collection: Iterable[(T, T)]): Option[(T, T)] = {
    var vek = Vector[(T, T)]()
    for(pair <- collection) {
      if(pair._1 == pair._2) vek = vek :+ pair
    }
    if(!vek.isEmpty) Some(vek(0))
    else None
  }
  ...
  def rotatingZip[A, B](alist: List[A], blist: List[B]): List[(A, B)] = {
    if(alist.length == blist.length) {
      val list = alist.zip(blist)
      list
    }
    else {
      val times = alist.length / blist.length
      var list = blist
      for(time <- 1 to times - 1) {
        list = list ++ blist
      }
      val result = alist.zip(list) 
      result
    }
  }
}
